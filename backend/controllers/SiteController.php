        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->username != NULL) {
                ...
                if ($dati_utente) {
                    ...
                    Yii::$app->getSession()->addFlash('success', Yii::t('amosplatform', 'Ti è stata inviata una mail contenente il link per scegliere la nuova password.'));
                    return $this->redirect('login');
                } else {
                    Yii::$app->getSession()->addFlash('danger', Yii::t('amosplatform', 'Attenzione. La username inserita non &egrave; associata a nessun profilo attivo!'));
                ...
                if ($dati_utente) {
                    ...
                    Yii::$app->getSession()->addFlash('danger', Yii::t('amosplatform', 'Attenzione. Il codice fiscale inserito non &egrave; associato a nessun profilo attivo!'));
                    return $this->redirect('forgot-password');
                }
            } else {
                Yii::$app->getSession()->addFlash('danger', Yii::t('amosplatform', 'Attenzione. E\' necessario compilare almeno uno dei due campi: username o codice fiscale!'));
                return $this->redirect('forgot-password');
            }
        } else {
